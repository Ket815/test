...
const dateTypeCard = document.getElementById('date-type-card');
...
// Dimensions captured once on first click (before transform: scale shrinks the button)
let noBtnW = 0;
let noBtnH = 0;

// Pin the No button to fixed viewport positioning on first click
function ensureFixedPosition() {
  if (isFixed) return;

  // Capture real unscaled size and viewport position while still in normal flow
  noBtnW = noBtn.offsetWidth || 80;
  noBtnH = noBtn.offsetHeight || 44;
  const rect = noBtn.getBoundingClientRect();

  // Switch to fixed — now that backdrop-filter is gone from the card,
  // position:fixed is correctly relative to the viewport
...
yesBtn.addEventListener('click', () => {
  ...
  // Play chime sequence immediately
  playSuccessSound();
  ...
  // Show the floating loader
  loaderModal.classList.remove('hidden');

  // Wait 5 seconds, then hide loader and show the time modal
  setTimeout(() => {
...
// Confirm Time button click
confirmTimeBtn.addEventListener('click', () => {
  if (!startTimeInput.value || !endTimeInput.value) {
    alert("Please let me know your free times first! ⏰");
    return;
  }
  ...
  // Transition to Date Type Card
  timeCard.classList.add('hidden');
  dateTypeCard.classList.remove('hidden');
});

// Date Type button click
dateBtns.forEach(btn => {
  btn.addEventListener('click', (e) => {
    selectedDateType = e.target.getAttribute('data-type');

    // Hide Modal and trigger final text display
    schedulingModal.classList.add('hidden');
    triggerFinalSuccess();

    // Format the WhatsApp message
    const message = `Hey! I'm free from ${selectedStartTime} to ${selectedEndTime}, let's do ${selectedDateType}! 💖`;
    const encodedMessage = encodeURIComponent(message);

    // IMPORTANT: Replace '1234567890' with your actual phone number.
    // Include the country code but do NOT include the '+' sign or any spaces/dashes.
    // Example: For US number (123) 456-7890, use '11234567890'.
    const phoneNumber = '57986157';
    const whatsappUrl = `https://wa.me/${57986157}?text=${encodedMessage}`;

    // Open WhatsApp in a new tab after a tiny delay so they see the text first
    setTimeout(() => {
...
function triggerFinalSuccess() {
  // The success animation is already running. 
  // We just show the final special message.
